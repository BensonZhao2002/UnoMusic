...
public class Control : MonoBehaviour {
	...
	private float audioTimer = 0f; // Tracks elapsed time since the first audio started
	private bool timerStarted = false; // Ensures timer starts with the first audio
	private float audioLength = 0f; // Automatically determined audio length
	private double globalDSPStartTime = 0;
	...
	public void PlayCardAudio(string color, int toneIndex)
	{
		...
		if (clip != null)
		{
			// Initialize global timer if it's the first clip
			if (!timerStarted)
			{
				StartAudioTimeline(clip);
			}

			// Set the clip and align playback to global timer
	...
	private void SyncAllAudioSources()
	{
		...
		// Schedule all audio sources to play at the same DSP time
		melodySource.SetScheduledStartTime(dspTime + 0.1); // Slight buffer to ensure sync
		melodySource.time = syncTime;

		chordSource.SetScheduledStartTime(dspTime + 0.1);
		chordSource.time = syncTime;

		bassSource.SetScheduledStartTime(dspTime + 0.1);
		bassSource.time = syncTime;

		drumSource.SetScheduledStartTime(dspTime + 0.1);
		drumSource.time = syncTime;

		//Debug.Log($"Synchronized all audio to {syncTime} seconds.");
	}
	...
	private void ToggleScaleMode()
	{
		...
		foreach (var color in keys)
		{
			...
			{
				//Debug.Log($"Reloading {color} with tone index {toneIndex}");
				PlayCardAudio(color, toneIndex);
			}
		}
	}
}
